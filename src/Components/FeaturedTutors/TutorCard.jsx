...
import { MdOutlineComputer, MdOutlineErrorOutline, MdOutlineSafetyCheck } from "react-icons/md";
...
const TutorCard = ({ tutor }) => {
  ...
  <div className="px-6 py-4 space-y-1.5">
    ...
    <div className="text-[#5B5C77] space-y-1.5 px-6 mx-auto">
        <p className="flex items-center gap-1.5 "><IoLocationOutline />{tutor.location}</p>
        <p className="flex items-center gap-1.5 "><FaRegClock/>{tutor.timeSlot}</p>
        <p className="flex items-center gap-1.5 "><MdOutlineComputer/>{tutor.teachingMode}</p>

        <div>
           {isBooked ? <span className="text-red-400 text-[17px] flex items-center gap-1.5 "><MdOutlineErrorOutline />fully booked</span> : ""}
        </div>
 ...
 <hr  className="w-full mb-5 mt-7 border-white/10"/>
    ...
    <div className="px-6 mx-auto flex justify-between">
        <h1><span className="font-serif font-semibold text-3xl">৳</span><span className="font-serif font-extrabold text-3xl">{tutor.pricePerHour}</span><span className="font-normal text-[#5B5C77] text-xl font-sans">/hr</span></h1>

        <Link href={''} className="inline-flex items-center gap-1.5 rounded-2xl border border-[#0DBF82] bg-[#0DBF82] px-4 py-2 text-[13px] font-semibold text-[#081A12] transition-all duration-150 hover:border-[#2DE8A8] hover:bg-[#2DE8A8] capitalize"
